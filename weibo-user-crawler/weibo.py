...
class Weibo(object):
    ...
    def parse_weibo(self, weibo_info):
        ...
        # 调用获取评论信息的方法
        # weibo["comments"] = "hello"

        weibo["comments"] = self.get_comments(weibo["id"], max_count=1000, start_page=1)
    ...
    def get_comments(self, weibo_id, max_count=1000, start_page=1):
        # 定义一个回调函数，用于处理下载完成后的评论
        def on_downloaded(weibo, comments):
            # 在这里可以对下载的评论进行处理，比如保存到文件或者数据库中
            print("下载完成，微博ID：", weibo["id"])
            if comments:
                print("评论列表：", comments)
            else:
                print("该微博暂无评论")
        ...
        # 调用函数进行评论下载，并获取结果
        comments = self._get_weibo_comments_nocookie({"id": weibo_id}, 0, max_count, start_page, on_downloaded)
        # print(comments)
        if comments is not None:
            for comment in comments:
                if self.remove_html_tag:
                    # 去除标签后的文本
                    text_without_tags = re.sub('<[^<]+?>', '', comment["text"]).replace('\n', '').strip()
                    # 提取img标签中的alt属性值作为评论文本
                    text_with_alt = re.findall(r'<img alt=(.*?) src=.*?>', comment["text"])
                    if text_with_alt:
                        alt_text = text_with_alt[0]
                        # 如果提取到了alt属性值，则将其加入文本中
                        comment["text"] = f"{text_without_tags} {alt_text}"
                    else:
                        # 如果未提取到alt属性值，则只使用去除标签后的文本
                        comment["text"] = text_without_tags
                else:
                    comment["text"] = comment["text"]
                if "reply_text" in comment:
                    if self.remove_html_tag:
                        # 去除标签后的文本
                        text_without_tags2 = re.sub('<[^<]+?>', '', comment["reply_text"]).replace('\n', '').strip()
                        # 提取img标签中的alt属性值作为评论文本
                        text_with_alt2 = re.findall(r'<img alt=(.*?) src=.*?>', comment["reply_text"])
                        if text_with_alt2:
                            alt_text2 = text_with_alt2[0]
                            # 如果提取到了alt属性值，则将其加入文本中
                            comment["reply_text"] = f"{text_without_tags2} {alt_text2}"
                        else:
                            # 如果未提取到alt属性值，则只使用去除标签后的文本
                            comment["reply_text"] = text_without_tags2
                    else:
                        comment["reply_text"] = comment["reply_text"]

            def remove_fields_recursive(obj, fields_to_remove):
                if isinstance(obj, dict):
                    for key in list(obj.keys()):
                        if key in fields_to_remove:
                            del obj[key]
                        else:
                            remove_fields_recursive(obj[key], fields_to_remove)
                elif isinstance(obj, list):
                    for item in obj:
                        remove_fields_recursive(item, fields_to_remove)
            ...
            for comment in comments:
                remove_fields_recursive(comment, fields_to_remove)
    ...
    #         # 打印评论到控制台
    #     # print("评论：______________")
    #     # print("comments:")
    #     # print(comments)
    #     # comments2 = []
    #     # for comment in comments:
    #     #     data = self.parse_sqlite_comment(comment, weibo)
    #     #     comments2.append(data)
    #     # print("comments2:")
    #     # print(comments2)
    #     cur_count += count
    #     page += 1
    ...
    #     if page > req_page:
    #         return comments
    #     return self._get_weibo_comments_nocookie(
    ...
    def _get_weibo_comments_nocookie(
        ...
        weibo_id = weibo["id"]
        url = "https://m.weibo.cn/api/comments/show?id={}&page={}".format(weibo_id, page)
        req = requests.get(url)
        try:
            json_data = req.json()
        except Exception as e:
            logger.warning("未能抓取完整评论 微博id: {}".format(weibo_id))
            return

        data = json_data.get("data")
        if data is None:
            logger.warning("未找到评论数据 微博id: {}".format(weibo_id))
            return

        comments = data.get("data")
        if comments is None:
            logger.warning("未找到评论列表 微博id: {}".format(weibo_id))
            return
        ...
        req_page = data.get("max")
        if req_page is None or req_page == 0:
            return []
    ...
    def parse_comment_content(self, comment, weibo):
        if not comment:
            return
        comment_content = OrderedDict()
        comment_content["id"] = comment["id"]
        comment_content["weibo_id"] = weibo["id"]

        comment_content["user_id"] = comment["user"]["id"]
        comment_content["user_screen_name"] = comment["user"]["screen_name"]
        if self.remove_html_tag:
            # 去除标签后的文本
            text_without_tags = re.sub('<[^<]+?>', '', comment["text"]).replace('\n', '').strip()
            ...
            if text_with_alt:
                alt_text = text_with_alt[0]
                # 如果提取到了alt属性值，则将其加入文本中
                comment_content["text"] = f"{text_without_tags} {alt_text}"
            else:
                # 如果未提取到alt属性值，则只使用去除标签后的文本
                comment_content["text"] = text_without_tags
        else:
            comment_content["text"] = comment["text"]
        # print(comment_content["text"])
        self._try_get_value("like_count", "like_count", comment_content, comment)
    ...
    def parse_sqlite_comment(self, comment, weibo):
        ...
        if self.remove_html_tag:
            ...
            if text_with_alt:
                alt_text = text_with_alt[0]
                # 如果提取到了alt属性值，则将其加入文本中
                sqlite_comment["text"] = f"{text_without_tags} {alt_text}"
            else:
                # 如果未提取到alt属性值，则只使用去除标签后的文本
                sqlite_comment["text"] = text_without_tags
        else:
